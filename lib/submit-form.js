...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!user?.email_confirmed_at || user.is_anonymous || normalizeEmail(user.email) !== normalizeEmail(sanitized.email)) {
    ...
    const ledgerRow = {
      submission_id: sanitized["submission-id"],
      user_id: user.id,
    ...
    const { data: app, error: ledgerError } = await admin
      .from("applications")
      .upsert(ledgerRow, { onConflict: "submission_id" })
      .select("id,submission_id,status,created_at,updated_at")
      .single();
